...
def clean_named_index(name):
    """Remove the optional 'Priyam -' prefix while preserving the index name."""
    return re.sub(r'^\s*Priyam\s*-\s*', '', str(name), flags=re.IGNORECASE).strip()


def read_named_indices(book, master_dates):
    """Find and read a worksheet whose row 1 contains Date and Priyam index headers.

    Every non-Date column on a detected sheet is treated as an additional Index
    series. Values are joined to the workbook by actual dates, not row numbers.
    """
    detected = []
    frames = []
    starts = {}
    ...
    for sheet in book.sheets:
        first_row = book.cells(sheet, last_row=1)
        headings = header_columns(first_row)
        if not headings:
            continue

        date_cols = [col for col, value in headings.items()
                     if str(value).strip().lower() == 'date']
        has_priyam = any(
            re.match(r'^\s*Priyam\s*-', str(value), flags=re.IGNORECASE)
            for value in headings.values()
        )
        if not date_cols or not has_priyam:
            continue
        ...
        for address, value in cells.items():
            ...
            if match and int(match.group(1)) >= 2:
                try:
                    ...
                    continue
        if not rows:
            raise InputError(f'{sheet}: the Date column contains no valid Excel dates.')

        renamed = {}
        for col, heading in headings.items():
            if col == date_col:
                continue
            cleaned = clean_named_index(heading)
            if cleaned:
                if cleaned in renamed.values():
                    raise InputError(f'{sheet}: duplicate index name after removing Priyam -: {cleaned}')
                renamed[col] = cleaned

        columns = {}
        date_index = list(rows.values())
        for col, cleaned in renamed.items():
            values = [cells.get(f'{col}{row}') for row in rows]
            columns[cleaned] = pd.to_numeric(
                pd.Series(values, index=date_index), errors='coerce'
            )
        frame = pd.DataFrame(columns).sort_index()
        if frame.index.has_duplicates:
            raise InputError(f'{sheet}: duplicate dates are not allowed.')
        if (frame.notna() & (frame <= 0)).any().any():
            raise InputError(f'{sheet}: index values must be positive numbers.')

        for name in frame.columns:
            first = frame[name].first_valid_index()
            if first is None:
                raise InputError(f'{sheet}: no numeric values found for {name}.')
            starts[name] = first

        # Use the existing workbook trading-date spine. Missing named-index
        # observations are forward-filled later by adjusted_prices().
        frames.append(frame.reindex(master_dates))
        detected.append(sheet)

    if not frames:
        return pd.DataFrame(index=master_dates), {}, []

    combined = pd.concat(frames, axis=1)
    if combined.columns.duplicated().any():
        duplicates = combined.columns[combined.columns.duplicated()].unique()
        raise InputError('Named indices appear more than once: ' + ', '.join(duplicates))
...
def read_model(content):
    ...
    # Optional user-maintained named-index sheet. A sheet is detected when row 1
    # contains a Date column and at least one header beginning with "Priyam -".
    # All its series are added to Index mode and the prefix is removed.
    named_index, named_starts, named_sheets = read_named_indices(book, index.index)
    for name in named_index.columns:
        # The named sheet is authoritative if its cleaned name already exists.
        index[name] = named_index[name]
    ...
    # For named indices, the first numeric observation is the declared start.
    declared['Index'].update(named_starts)

    # Every named index uses face value 10, as requested.
    face.update({name: 10.0 for name in named_index.columns})

    # Make all newly detected indices available in the allocation editor with
    # a default weight of zero. Existing allocation rows are not duplicated.
    existing_names = set(weights['Index'])
    additions = [
        {'Index': name, 'Weight (%)': 0.0}
        for name in named_index.columns if name not in existing_names
    ]
    if additions:
        weights = pd.concat([weights, pd.DataFrame(additions)], ignore_index=True)
...
def simulate(prices, weights, amount, schedule, instrument_type):
    ...
    initial_units = float(amount) * target / p[0]
    held = np.floor(initial_units) if is_etf else initial_units
    cash = float(amount) - float(held @ p[0]) if is_etf else 0.0
    total = float(amount)
    daily=[]; unit_rows=[]; mv_rows=[]; weight_rows=[]; trades=[]
    schedule = set(schedule)
    for j,name in enumerate(names):
        trades.append({'Pricing date':dates[0], 'Effective date':dates[0],
            'Event':'Initial investment', 'Index':name,
            'Portfolio before (₹)':total, 'Target weight (%)':target[j]*100,
            'Price used':p[0,j], 'Old units':0.0, 'New units':held[j],
            'Unit change':held[j], 'Trade value (₹)':held[j]*p[0,j],
            'Cash after (₹)':cash,
            'Unit method':'Whole units' if is_etf else 'Fractional units'})
    for i, day in enumerate(dates):
        if i > 0 and dates[i-1] in schedule:
            old = held.copy()
            desired_units = total * target / p[i-1]
            held = np.floor(desired_units) if is_etf else desired_units
            cash = total - float(held @ p[i-1]) if is_etf else 0.0
            for j,name in enumerate(names):
                trades.append({'Pricing date':dates[i-1], 'Effective date':day,
                    'Event':'Rebalance', 'Index':name,
                    'Portfolio before (₹)':total, 'Target weight (%)':target[j]*100,
...
def return_rate(start_value, end_value, days, annualise=False):
    if days <= 0 or start_value <= 0:
        return np.nan
    ratio = end_value/start_value
    return ratio**(365/days)-1 if annualise else ratio-1
...
def summary_tables(model, selected, buyhold, prices, amount, requested_end, vol_mode):
    ...
    def block(label, series, daily_returns, portfolio, ddof):
        ...
        for period,months in PERIODS:
            ...
            if valid:
                # Fixed windows of 1 year or longer are CAGR periods even when
                # working-day rollback makes the observed span 363/364 days.
                # Since Inception uses the requested calendar dates to decide.
                annualise = (months is not None and months >= 12) or (
                    months is None and requested_end >= start + pd.DateOffset(years=1)
                )
                ret=return_rate(
                    float(series.at[anchor]), float(series.at[end]),
                    (end-anchor).days, annualise=annualise
                )
    ...
    windows=pd.DataFrame([{'Period':label,'Start date':anchor,'End date':end,
        ...
        'Return method':(
            'CAGR' if ((months is not None and months >= 12) or
                       (months is None and requested_end >= start + pd.DateOffset(years=1)))
            else 'Absolute'
        ),
        'Available':anchor is not None and start<=anchor<end and anchor in prices.index}
        for (label,months),anchor in zip(PERIODS,anchors.values())])
...
def calculate(model, config, allocation, vol_mode=VOL_MODES[0]):
    ...
    instrument_type=config.get('instrument_type','Non-ETF')
    if instrument_type not in ('ETF','Non-ETF'):
        raise InputError('Investment vehicle must be ETF or Non-ETF.')
    selected=simulate(prices,weights,amount,schedule,instrument_type)
    buyhold=simulate(prices,weights,amount,[],instrument_type)
    ...
    checks=pd.DataFrame([
        ...
        {'Check':'Unit and cash method',
         'Result':('Whole units with residual cash' if instrument_type=='ETF'
                   else 'Fractional units; cash fixed at zero'),
         'Status':('PASS' if selected.daily['Cash (₹)'].min()>=-1e-6
                   and (instrument_type=='ETF' or selected.daily['Cash (₹)'].abs().max()<1e-8)
                   else 'FAIL')},
    ])
...
An optional named-index worksheet is detected automatically when row 1 contains **Date**
and at least one column beginning with **Priyam -**. The prefix is removed, all non-Date
columns on that sheet become Index-mode series, their first numeric observations become
their starting dates, and face value 10 is applied. The data is joined by actual date.
...
### 5. ETF versus Non-ETF unit treatment
The user chooses the investment vehicle before calculating.

**Non-ETF / Index portfolio:** fractional units are allowed:

`New units[i] = portfolio value × target weight[i] / price[i]`

The portfolio is fully invested and cash is exactly zero at inception and every rebalance.

**ETF portfolio:** only whole units are allowed:

`New units[i] = floor(portfolio value × target weight[i] / price[i])`

`Cash = portfolio value − sum(new units[i] × price[i])`

ETF residual cash remains uninvested and earns no interest. Initial holdings are purchased
on the effective investment date. Rebalances use the scheduled pricing date and take effect
on the following available date.
...
`Absolute return = ending value / starting value − 1` for 1, 3 and 6 month windows.
...
follow the same period rule, including lookbacks that cross leap years.

### 8A. Download audit trail
Every CSV download begins with the selected input values, followed by the complete portfolio
allocation and then the requested summary, source or daily-calculation table. This makes each
download reproducible without referring back to the screen.
...
def input_table(config, allocation, result=None, vol_mode=None):
    """Create an auditable table of inputs included in every download."""
    rows = [
        ('Investment amount (₹)', config.get('amount')),
        ('Requested investment date', config.get('start')),
        ('Requested as-on date', config.get('end')),
        ('Investment vehicle', config.get('instrument_type')),
        ('Unit treatment', 'Whole units with residual cash'
         if config.get('instrument_type') == 'ETF'
         else 'Fractional units; fully invested; no cash'),
        ('Series based on', config.get('mode')),
        ('Rebalancing frequency', config.get('frequency')),
        ('Custom interval', config.get('custom_step')
         if config.get('frequency') == 'Custom' else 'Not applicable'),
        ('Custom interval unit', config.get('custom_unit')
         if config.get('frequency') == 'Custom' else 'Not applicable'),
        ('Volatility calculation', vol_mode or ''),
    ]
    if result is not None:
        rows.extend([
            ('Effective investment date', result.get('start')),
            ('Effective as-on date', result.get('end')),
            ('Effective rebalance calendar', result.get('schedule_frequency')),
        ])
    return pd.DataFrame(rows, columns=['Input', 'Value'])


def csv_with_inputs(frame, config, allocation, title, result=None, vol_mode=None):
    """Export inputs, allocation and the requested dataset in one CSV file."""
    output = io.StringIO()
    output.write('INPUT VALUES\n')
    input_table(config, allocation, result, vol_mode).to_csv(output, index=False)
...
def main():
    ...
    if model.get('simple_workbook'):
        ...
        st.success(
            ...
            '. The Priyam - prefix was removed automatically.'
        )
    with st.sidebar:
        ...
        with st.form('portfolio_inputs'):
            vehicle_label=st.radio(
                'Investment vehicle',
                ['Non-ETF / Index portfolio', 'ETF portfolio'],
                index=0,
                help=(
                    'Non-ETF uses fractional units and stays fully invested with zero cash. '
                    'ETF uses whole units and keeps the uninvested remainder as cash.'
                ),
                key='input_vehicle'
            )
            instrument_type='ETF' if vehicle_label=='ETF portfolio' else 'Non-ETF'
    ...
    if result is not None:
        with summary_tab:
            ...
            st.markdown(
                ...
                f'<div style="color:#c4bba7;margin-top:6px">{choice_label} · {instrument_type} · {mode} series · '
            ...
            if amount<100000 and instrument_type=='ETF':
                st.warning('The workbook recommends an investment of at least ₹1 lakh. Smaller amounts can create more rounding cash.')
            if instrument_type=='Non-ETF':
                st.info('Non-ETF mode uses fractional units, remains fully invested and keeps cash at exactly zero.')
            ...
            inception_is_cagr = pd.Timestamp(end) >= result['start'] + pd.DateOffset(years=1)
            metrics[2].metric('CAGR' if inception_is_cagr else 'Absolute return',f'{value:.2%}')
            ...
            st.caption(
                ...
                "1 Year and longer fixed windows use CAGR; shorter windows use absolute returns. "
            ...
            st.download_button(
                ...
                csv_with_inputs(
                    displayed_summary, config, allocation,
                    'Formatted summary', result, vol_mode
                ),
    ...
    with data_tab:
        ...
        show_table(st,frame)
        st.download_button('Download this original-data page (CSV)',
                           csv_with_inputs(frame,config,allocation,'Original data page',result,vol_mode),
                           'original_data_page.csv','text/csv')
        with st.expander('Download complete source price tables'):
            for source in model['raw']:
                st.download_button(
                    f'Download complete {source} source (CSV)',
                    csv_with_inputs(model['raw'][source],config,allocation,
                                    f'Complete {source} source',result,vol_mode),
                    f'original_{source.lower()}_prices.csv','text/csv'
                )
    with explanation_tab:
        ...
        if result is not None:
            ...
            inception_is_cagr = pd.Timestamp(end) >= result['start'] + pd.DateOffset(years=1)
            equation=(f'({endvalue:.8f} / {amount:.8f}) ** (365 / {days}) - 1'
                      if inception_is_cagr else f'{endvalue:.8f} / {amount:.8f} - 1')
    ...
    with daily_tab:
        if result is None:
            ...
            st.download_button('Download complete calculation table (CSV)',
                               csv_with_inputs(frame,config,allocation,
                                               dataset,result,vol_mode),
